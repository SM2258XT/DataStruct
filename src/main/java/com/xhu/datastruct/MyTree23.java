...
public class MyTree23 {
    ...
    public List<Integer> getDataArray(Node23 node, int level, int childNum){
        List<Integer> list = new ArrayList<Integer>();
        int[] array = node.getNodeData();
        for (int i : array)
            list.add(i);
        ...
        for (int i = 0; i < node.getItemNums() + 1; i++) {
            Node23 child = node.getChild(i);
            if (child != null){
                List<Integer> list1 = getDataArray(child, level + 1, i);
                list.addAll(list1);
            }
            else
                return list;
    ...
    private static class Node23 {
        ...
        private final Node23[] childArray = new Node23[N];
        private final DataStore[] itemArray = new DataStore[N - 1];
        ...
        public int[] getNodeData(){
            int[] data = new int[itemNums];
            for (int i = 0; i < itemNums; i++)
                data[i] = itemArray[i].getData();
            return data;
        }
    }
